...
### `review_features_v1.csv`

Deterministic downstream artifact generated by
`scripts/build_ds_dataset.py`. Each canonical review produces one row with a
rating-derived weak label, text-length and timestamp features, quality-flag
indicators, and simple versioned issue-keyword signals. The column contract and
label limitations are documented in
`docs/weak_labeling_strategy_v1.md`. Reviewer identity fields remain excluded.
